...
func TestSchnorrSign(t *testing.T) {
	for _, test := range bip340TestVectors {
		if len(test.secretKey) != 0 {
			d := decodeHex(test.secretKey)
			privKey, _ := PrivKeyFromBytes(S256(), d)

			aux := decodeHex(test.auxRand)
			msg := decodeHex(test.message)

			sig, err := privKey.SchnorrSign(msg, aux)
			if err != nil {
				t.Fail()
			}
			...
			verify, err := schnorrVerify(msg, pubKey, sig[:])
			if err != nil {
				t.Fail()
			}
...
func TestParseSchnorrPubKey(t *testing.T) {
	// Test parsing the test vector public keys.
	for _, test := range bip340TestVectors {
		_, err := ParseSchnorrPubKey(decodeHex(test.publicKey))
		if err != nil && test.validPubKey {
			t.Fail()
		}

		// If the test has a private key, then test the serialization of the public key.
		if len(test.secretKey) != 0 {
			privKey, _ := PrivKeyFromBytes(S256(), decodeHex(test.secretKey))
			pubKey := privKey.SchnorrPubKey()

			if !bytes.Equal(decodeHex(test.publicKey), pubKey.Serialize()) {
				t.Fail()
			}
		}
	}

	// Test conversion of a compressed public key to a schnorr public key.
	{
		for _, test := range pubKeyTests {
			key, err := ParseSchnorrPubKey(test.key)
			if err != nil && test.format == pubkeyCompressed {
				t.Fail()
			}
